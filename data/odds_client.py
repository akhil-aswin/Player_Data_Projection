"""
Pulls player prop odds from multiple sportsbooks via The Odds API.
Works for any sport — pass the sport key at call time.
Requires ODDS_API_KEY as an environment variable.
"""

import requests
from config import (
    ODDS_API_KEY, ODDS_API_BASE_URL, NBA_SPORT_KEY,
...
class OddsAPIClient:
    ...
    def get_upcoming_events(self, sport: str = NBA_SPORT_KEY) -> list:
    ...
    def extract_player_lines(props_response: dict, player_name: str,
                             market: str) -> list:
        """
        Flattens the API response into:
